...
OGLExceptions::OGLExceptions(const QString & msg) {
    m_msgStack.push_back( std::make_pair(QString(msg), QString()));
}

OGLExceptions::OGLExceptions(const QString & msg, const QString & where) {
    m_msgStack.push_back( std::make_pair(QString(msg), where));
}

OGLExceptions::OGLExceptions(OGLExceptions & previous, const QString & msg) :
    m_msgStack(previous.m_msgStack)
{
    m_msgStack.push_back( std::make_pair(QString(msg), QString()));
}

OGLExceptions::OGLExceptions(OGLExceptions & previous, const QString & msg, const QString & where)  :
    m_msgStack(previous.m_msgStack)
{
    m_msgStack.push_back( std::make_pair(QString(msg), where));
}

void OGLExceptions::writeMsgStackToStream(std::ostream & strm) const {
