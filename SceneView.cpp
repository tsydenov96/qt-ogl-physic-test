...
void SceneView::initializeGL() {
    FUNCID(SceneView::initializeGL);
        ...
        m_sphereObject.create(SHADER(0));
        ...
        throw OGLExceptions(ex, "OpenGL initialization failed.", FUNC_ID);
    }
}
...
void SceneView::paintGL() {
    ...
    if (((DebugApplication *)qApp)->m_aboutToTerminate){
        qDebug() << "aboutToTerminate" <<((DebugApplication *)qApp)->m_aboutToTerminate << '\n';
        //return;
    }
    ...
    // set the background color = clear color
    QVector3D backColor(0.1f, 0.2f, 0.3f);
...
void SceneView::checkInput() {
    ...
    if (m_keyboardMouseHandler.buttonDown(Qt::RightButton)) {
        ...
        if (m_keyboardMouseHandler.keyDown(Qt::Key_W) ||
            ...
            m_keyboardMouseHandler.keyDown(Qt::Key_Control) ||
            m_keyboardMouseHandler.keyDown(Qt::Key_Space))
        {
...
void SceneView::processInput() {
    ...
    if (m_keyboardMouseHandler.buttonDown(Qt::RightButton)) {
        ...
        if (m_keyboardMouseHandler.keyDown(Qt::Key_S))
            translation -= m_camera.forward();
        if (m_keyboardMouseHandler.keyDown(Qt::Key_A))
            translation -= m_camera.right();
        if (m_keyboardMouseHandler.keyDown(Qt::Key_D))
            translation += m_camera.right();
        if (m_keyboardMouseHandler.keyDown(Qt::Key_Control))
            translation -= m_camera.up();
        if (m_keyboardMouseHandler.keyDown(Qt::Key_Space))
            translation += m_camera.up();
